...
const TopTenCard = ({ item, index }) => {
    ...
    return (
        <motion.div
            ...
            className="relative min-w-[110px] h-[160px] sm:min-w-[220px] sm:h-[320px] rounded-2xl overflow-hidden cursor-pointer group"
        >
            <img
                src={item.cover_url}
                alt={item.title}
            ...
            <div className="absolute top-3 left-3 w-9 h-9 rounded-full flex items-center justify-center text-[10px] font-bold text-white  shadow-lg" style={{ background: unread(item.chapters) ? "blue" : "transparent" }}>
                {unread(item.chapters)}
            </div>
            ...
            <div className="absolute inset-0 bg-black/75 opacity-0 group-hover:opacity-100 transition duration-300 p-4 flex flex-col justify-end">
                ...
                <div className="flex items-center gap-2 text-xs text-gray-300 mt-1">
                    ...
                    <span
                        className={`px-2 py-0.5 rounded-full text-[10px] ${item.status === "reading"
                            ? "bg-green-600"
                            : "bg-gray-600"
                            }`}
                    >
            ...
            <div className="flex items-center gap-4 px-8 py-5">
                ...
                >
                    <div className="flex items-center bg-white/10 rounded-full px-5 py-3 border border-white/10">
                        ...
                        <form onSubmit={() => navigate(`/search/${searchQuery}`)} className="flex">
                            <input
                                placeholder="manhwa..."
                                value={searchQuery}
                                onChange={(e) => {
                            ...
                            />
                            <button type="submit" className="absolute end pr-5 ">Search</button>
                        </form>
                    </div>
